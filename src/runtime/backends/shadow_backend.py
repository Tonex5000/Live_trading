...
from src.models import DecisionEvent, Signal
...
class ShadowBackend(RuntimeBackend):
    """Shadow runtime path: evaluate strategy + risk and persist diagnostics only.

    It intentionally does not execute trades or mutate paper positions/trades.
    """

    def __init__(self, *, symbol: str, timeframe: str, strategy, risk_manager: RiskManager):
        self.symbol = symbol
    ...
    def on_new_bar(self, df_window: pd.DataFrame) -> None:
        ...
        try:
            ...
            self._latest_signal = {
                ...
                "execution_reason": "shadow_no_execute",
        ...
        with session_scope() as session:
            session.add(
                Signal(
                    ...
                    position_size=0.0,
                    buffer_size=len(df_window),
                    reason=f"strategy={signal_obj.reason};risk={risk.reason};exec=shadow_no_execute",
                    reason_code=risk.reason,
                    explanation=risk.explanation,
                    expected_value=risk.expected_value,
                    expected_rr=risk.expected_rr,
    ...
    def health(self, buffer_size: int) -> Dict[str, Any]:
        return {"status": "ok", "buffer_size": buffer_size, "mode": "shadow_live"}

    def metrics(self) -> Dict[str, Any]:
        with session_scope() as session:
            total_decisions = len(session.exec(select(DecisionEvent)).all())
            rejected = len(session.exec(select(DecisionEvent).where(DecisionEvent.approved == False)).all())
            approved = total_decisions - rejected
